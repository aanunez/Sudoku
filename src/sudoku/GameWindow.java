...
public class GameWindow {
	...
	private void initialize() {
            ...
            final Map<Integer, JFormattedTextField> map = new HashMap<Integer, JFormattedTextField>();
            for(int i=0; i<array.length; i++)
                map.put(i, array[i]);
                ...
                public void actionPerformed(ActionEvent e) {  
                    for(int i = 0; i < 81; i++){
                        ...
                        if(map.get(i).isEditable() && !map.get(i).getText().trim().isEmpty() && Character.isDigit(map.get(i).getText().trim().charAt(0)) ){
                            if((Integer.valueOf(map.get(i).getText().trim().charAt(0))-puzzle.solu[(int) i/9][i%9])-48 == 0){
                                puzzle.grid[(int) i/9][i%9]=Integer.valueOf(map.get(i).getText().trim().charAt(0)-48);
                                map.get(i).setText(" "+map.get(i).getText().trim().charAt(0));
                                map.get(i).setForeground(Color.GREEN);map.get(i).setEditable(false);
                            }
                            else
                                map.get(i).setForeground(Color.BLUE); //if the input is incorrect, it sets the color of the number to blue
                            map.get(i).setText(" "+map.get(i).getText().trim().charAt(0));
                        }
                    }
            ...
            ImagePanel panel = new ImagePanel(new ImageIcon("pics/grid.jpg").getImage());
            panel.setFocusable(false);
            panel.setBounds(46, 46, 361, 370);
            frmSudoku.getContentPane().add(panel);

            //Sets a background image for GameWindow
            ImagePanel backGndPanel = new ImagePanel(new ImageIcon("Pics/background.png").getImage());
